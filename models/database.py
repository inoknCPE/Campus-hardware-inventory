...
if hasattr(psycopg, "Cursor"):
    _orig_cursor_execute = psycopg.Cursor.execute

    def _postgres_compatible_execute(self, query, params=None, *args, **kwargs):
        if params is not None and "?" in str(query):
            query = str(query).replace("?", "%s")
            return _orig_cursor_execute(self, query, params, *args, **kwargs)
        if params is None and "?" in str(query):
            query = str(query).replace("?", "%s")
            return _orig_cursor_execute(self, query, *args, **kwargs)
        if "datetime('now')" in str(query):
            query = str(query).replace("datetime('now')", "CURRENT_TIMESTAMP")
            return _orig_cursor_execute(self, query, params, *args, **kwargs)
        return _orig_cursor_execute(self, query, params, *args, **kwargs)

    psycopg.Cursor.execute = _postgres_compatible_execute
